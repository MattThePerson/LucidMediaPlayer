...
| `storage.go` | Platform-specific AppData dir (`SunsetVideoPlayer`); `RecentEntry` type. **No build constraint** |
...
| `%APPDATA%\SunsetVideoPlayer\db.sqlite` | SQLite database — videos table (hash, filepath, last_pos, etc.) |
| `%APPDATA%\SunsetVideoPlayer\config\` | Future: `preferences.json`, `keybinds.json` |
| `%APPDATA%\SunsetVideoPlayer\media\<hash>\` | Future: seek thumbnails, waveform data per video |
...
The AppData directory is `SunsetVideoPlayer` (no spaces). The window title is `"Sunset Video Player"` (with spaces). Do not confuse the two — the folder name must not have spaces.
